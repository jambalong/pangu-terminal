# Credits

## Framework & Libraries
- [Rails 8](https://rubyonrails.org)
- [Hotwire](https://hotwired.dev)
- [RubyLLM](https://rubyllm.com) - LLM integration

## Deployment
- [Kamal 2](https://kamal-deploy.org/)
- [DigitalOcean](https://www.digitalocean.com/)

## Game Data
- [Wuthering Waves](https://wutheringwaves.com) by Kuro Games
- [Wuthering Waves Fandom Wiki](https://wutheringwaves.fandom.com/wiki/Wuthering_Waves_Wiki)

## Assets
- Material, character, and weapon icons: [Wuthering Waves Wiki](https://wuwa.wiki/en)

## Color Scheme
- [Catppuccin](https://catppuccin.com)

## Fonts
- [Oxanium](https://fonts.google.com/specimen/Oxanium) - Google Fonts
- [JetBrains Mono](https://fonts.google.com/specimen/JetBrains+Mono) - Google Fonts
- [Space Mono](https://fonts.google.com/specimen/Space+Mono) - Google Fonts

## AI Assistance
- [Claude](https://claude.ai) (Anthropic) — rubber duck debugging, design discussions, and scaffolding reference code throughout development
- [Gemini](https://ai.google.dev) (Google) — LLM provider for the Farming Advisor feature via RubyLLM
